...
try :
    ...
    eligibility_filename = 'Cal-OptimaEligibilityResponse.xlsx'
    claims_filename = 'Cal-OptimaClaimsResponse.xlsx'
    eligibility_data_list = []
    claims_data_list = []
    ...
    for value in data_as_list:
        ...
        eligibility_data_list.append(responseEle[0] if responseEle else {})
        ...
        if response and 'items' in response:
            for item in response['items']:
                claims_data_list.append(item)
        # try :
        #     if response and 'items' in response and len(response['items']) > 0:
        #         dict.update(response['items'][0])
        #     if responseEle and len(responseEle) > 0:
        #         dict.update(responseEle[0])
        #     claims_data_list.append(dict)
        # except Exception as e:
        #     print("Error for ", value['policy_number'])

     #insert list in XlFiles
    print("Eligibility Data:", eligibility_data_list)
    print("Claims Data:", claims_data_list)

    try:
        df_eligibility = pd.json_normalize(eligibility_data_list, sep='_')
        df_claims = pd.json_normalize(claims_data_list, sep='_')

        df_eligibility.to_excel(eligibility_filename, index=False)
        df_claims.to_excel(claims_filename, index=False)
    except Exception as e:
        print('Exception at policy_number ', value['policy_number'])
